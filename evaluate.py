...
def eval_torjan(model, test_X, test_Y, y_target, pattern_dict, num_classes):
    cur_test_X = np.array([infect_X(img, y_target, num_classes, pattern_dict)[0] for img in np.copy(test_X)])
    trojan_succ = np.mean(np.argmax(model.predict(cur_test_X), axis=1) == y_target)
...
def eval_defense():
    ...
    for y_target in random.sample(target_ls, 3):
        RES[y_target] = {}
        trojan_succ = eval_trojan(new_model, test_X, test_Y, y_target, num_classes=model.num_classes,
                                      pattern_dict=pattern_dict)

        print("Target: {} - Trojan Succ: {}".format(y_target, trojan_succ))
